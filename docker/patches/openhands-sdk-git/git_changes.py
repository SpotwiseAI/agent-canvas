...
# Augments get_changes_in_repo() to attach `git diff --numstat` line counts
# (additions/deletions) to each GitChange. Injected over the baked SDK by
# docker/Dockerfile. Re-sync if config/defaults.json bumps the agentServer
# pin. Upstream source: openhands-sdk/openhands/sdk/git/git_changes.py
...
def _get_numstat_map(
    repo_dir: str | Path, ref: str
) -> dict[str, tuple[int | None, int | None]]:
    """Map each changed path to (additions, deletions) via git diff --numstat.

    Binary files report ``-``/``-`` (stored as ``None``). Best-effort for
    renames: when git emits ``old => new`` the new path is used as the key.
    Never raises — a failure here just means counts are omitted.
    """
    try:
        output = run_git_command(
            ["git", "--no-pager", "diff", "--numstat", ref], repo_dir
        )
    except GitCommandError as e:
        logger.warning(f"Failed to get git numstat for {repo_dir}: {e}")
        return {}
    ...
    for line in output.splitlines() if output else []:
        ...
        additions = None if add_raw == "-" else _safe_int(add_raw)
        deletions = None if del_raw == "-" else _safe_int(del_raw)
        if " => " in path:
            # "old => new" (and the "{a => b}/f" brace form falls through to
            # whatever follows the arrow — best effort, may not resolve).
            path = path.split(" => ")[-1].strip().rstrip("}")
...
def get_changes_in_repo(
    ...
    numstat = _get_numstat_map(validated_repo, ref)

    def _counts(path: str) -> tuple[int | None, int | None]:
        return numstat.get(path, (None, None))
